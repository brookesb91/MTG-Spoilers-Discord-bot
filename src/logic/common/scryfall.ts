...
const GET_CARD_ERROR =
  'Something went wrong searching for your card, please wait a moment and try again';
const GET_SET_ERROR =
  'Something went wrong getting data for that set, please wait a moment and try again';

export async function scryfallGetCard(
  name: string,
  callback: (card: ICard, attemptedName: string) => Promise<string>
): Promise<string> {
  let data = await makeScryfallAPICall(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (card) {
        ...
        Log(
          "Expected 'card' object from Scryfall but could not parse, got the following:"
        );
        Log(cardData);
...
export async function scryfallGetSet(
  set: string,
      ...
      if (cardlist) {
        if (cardlist.object === 'list') {
          ...
          if (!cardlist.total_cards || cardlist.total_cards <= 0) {
            ...
            Log(
              `${cardlist.data.length} cards were found with set code ${set} out of a total of ${cardlist.total_cards}`
            );
            resolve(cardlist);
          }
        } else {
          Log(
            `Expected \'list\' object from Scryfall but got type \'${cardlist.object}\', with the following data`
          );
          Log(cardlistData);
        ...
        Log(
          "Expected 'list' object from Scryfall but could not parse, got the following:"
        );
        Log(cardlistData);
        reject(GET_SET_ERROR);
      }
