...
global.bot.on(Discord.Events.ClientReady, function () {
  ...
  global.prefix = readPrefix(constants.BOTDEFAULTPREFIX);
  readWatchedSets();
});
      ...
      switch (cmd.toLowerCase()) {
          ...
          if (queryIndex > 0) {
            ...
            if (query) {
              ...
              message.channel.send(
                `You have to supply a query, like so:\n${global.prefix}get Sonic Assault`
              );
            }
          } else {
            message.channel.send(
        ...
        case 'query':
          if (permissions.checkPermissions(message)) {
            ...
            commands.queryCardsCommand(message.channel, query);
          }
          break;
        ...
        default:
          message.channel.send(
            `No command ${cmd} found, please check your spelling or use ${global.prefix}help for a list of possible commands.`
          );
          break;
      }
    } catch (error) {
